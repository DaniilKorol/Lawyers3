...
async def get_screen_order(message: Message):
	...
	await bot.state_dispenser.set(message.peer_id, RegData.PROMO)
	return 'Введите промокод (если нет, то ставьте "-"):'

@bot.on.private_message(state=RegData.PROMO)
async def get_screen_order(message: Message):
	ctx.set("promo", message.text)
	...
	text_message = f'Уважаемые, пришел новый заказ❗\n'
	for i in sql.execute(f"SELECT vk_id, nickname, group_user FROM users WHERE group_user = '8'"):
		text_message += f'@id{i[0]} ({i[1]}), '
	for i in sql.execute(f"SELECT vk_id, nickname, group_user FROM users WHERE group_user = '4'"):
		text_message += f'@id{i[0]} ({i[1]}), '
	text_message += f'\nЗаказ: №{number_order}\nNickname: {nickname} 🤵\nПричина задержания: {reason} ⛓️\nСрок задержания: {jailtime} ⌚\nДежурная часть: {from_where} 🏢\nПромокод: {promo}'

	KEYBOARD = (Keyboard(inline=True).add(Callback(f"Взять❗", payload={"peer": message.peer_id, 'type_btn':'en', 'number_order':number_order, 'guvd':from_where_2}), color=KeyboardButtonColor.POSITIVE).add(Callback(f"Отклонить❗", payload={"peer": message.peer_id, 'type_btn':'dis', 'number_order':number_order, 'guvd':from_where_2}), color=KeyboardButtonColor.NEGATIVE).get_json())
	sql.execute(f"INSERT INTO orders VALUES (?, ?, ?, ?, ?, ?, ?, ?)", (number_order, nickname, message.peer_id, reason, jailtime, from_where, promo, screen))
	db.commit()
...
#"peer": message.peer_id, 'type_btn':'en', 'number_order':number_order, 'guvd':from_where_2
#{"peer": message.peer_id, 'type_btn':'dis', 'number_order':number_order, 'guvd':from_where_2}

async def generator_text(number_order):
	for order in sql.execute(f"SELECT nickname, vk_id, reasons, jailtime, where_from, screen, promo FROM orders WHERE numbers = '{number_order}'"):
		text = f'Заказ: №{number_order}\nNickname: @id{order[1]} ({order[0]}) 🤵\nПричина задержания: {order[2]} ⛓\nСрок задержания: {order[3]} ⌚\nДежурная часть: {order[4]} 🏢\nПромокод: {order[6]}'
		return text

async def get_order_info(number_order, param):
	for order in sql.execute(f"SELECT {param} FROM orders WHERE numbers = '{number_order}'"):
		return order[0]
	...
	if sql.fetchone() is None:
		...
		for user in sql.execute(f"SELECT nickname, vk_id, group_user FROM users WHERE vk_id = '{event.object.user_id}'"):
			if type_btn == 'en':
				...
				text = await generator_text(number_order)
				...
				KEYBOARD = (Keyboard(inline=True)
					.add(Callback(f"Отказаться от заказа❗", payload={"peer": user_id_inlian, 'type_btn':'dis_proc', 'user_id_accepted': user_id_accepted, 'number_order': number_order}), color=KeyboardButtonColor.NEGATIVE)
					.add(Callback(f"Заказ выполнен❗", payload={"peer": user_id_inlian, 'type_btn':'сompleted', 'user_id_accepted': user_id_accepted, 'number_order': number_order}), color=KeyboardButtonColor.POSITIVE).row()
					.add(Callback(f"Уведомить ГУВД❗", payload={"peer": user_id_inlian, 'type_btn':'allow_guvd', 'user_id_accepted': user_id_accepted, 'guvd':guvd, 'number_order': number_order})).get_json())
				...
				guvd = event.object.payload["guvd"]
				text = await generator_text(number_order)
				nickname_order = await get_order_info(number_order, 'nickname')
